class StoreManagement:
    ...
    def add_product(self):
        ...
        while True:
            month = input("Enter the month (or 'done' to finish): ").strip().capitalize()
            if month.lower() == "done":
                break
    ...
    def update_sale_price(self):
        """Method 2: Update the sale price of a specific product for a given month."""
        shelf_name = input("Enter the shelf name: ").strip()
        if shelf_name not in self.store_data:
            print("⚠️ Shelf does not exist.")
            return
        ...
        if month not in self.store_data[shelf_name][product_name]["Cost Price"]:
            print("⚠️ No cost price data for this month.")
            return
    ...
    def update_sale_price_for_shelf(self):
        """Method 3: Update sale prices for all products in a shelf by a given percentage."""
        shelf_name = input("Enter the shelf name: ").strip()
        if shelf_name not in self.store_data:
            print("⚠️ Shelf does not exist.")
            return
    ...
    def set_category(self):
        """Method 3: Set or update the category of a product."""
        shelf_name = input("Enter the shelf name: ").strip()
        if shelf_name not in self.store_data:
            print("⚠️ Shelf does not exist.")
            return
    ...
    def reset_cost_price(self):
        """Method 5: Reset cost price with 0 for a given shelf, product, and month."""
        shelf_name = input("Enter the shelf name: ").strip()
        if shelf_name not in self.store_data:
            print("⚠️ Shelf does not exist.")
            return
    ...
    def get_min_max_price(self):
        """Method 6: Get the maximum or minimum cost price with the shelf name of a product."""
        product_name = input("Enter the product name: ").strip()
        ...
        if price_type not in ["max", "min"]:
            print("⚠️ Invalid choice. Please enter 'max' or 'min'.")
            return
        ...
        if min_max_price is not None:
            ...
            print(f"⚠️ No cost price data found for '{product_name}'.")

    def get_average_cost_sale_profit(self):
        """Method 7: Get the average cost, sale price, and profit for all products in a shelf for a specific month."""
        shelf_name = input("Enter the shelf name: ").strip()
        if shelf_name not in self.store_data:
            print("⚠️ Shelf does not exist.")
            return
        ...
        for product_name, product_data in self.store_data[shelf_name].items():
            if month in product_data["Cost Price"] and month in product_data["Sale Price"]:
                cost_prices = product_data["Cost Price"][month]
                sale_prices = product_data["Sale Price"][month]
                count += 1
        ...
        if count > 0:
            ...
            print(f"✅ Average Cost for '{shelf_name}' in {month}: {avg_cost}")
            print(f"✅ Average Sale Price for '{shelf_name}' in {month}: {avg_sale}")
            print(f"✅ Average Profit for '{shelf_name}' in {month}: {avg_profit}")
        else:
            print(f"⚠️ No data found for {month} in '{shelf_name}'.")

    def get_product_average_cost_sale_profit(self):
        """Method 8: Get the average cost, sale price, and profit for a specific product in a specific month."""
        product_name = input("Enter the product name: ").strip()
        ...
        if count > 0:
            ...
            print(f"⚠️ No data found for {product_name} in {month}.")

    def run(self):
        """Main menu for user interaction."""
        while True:
            print("\n🛒 Store Management System")
            print("1️⃣ Create Shelf")
            print("2️⃣ Add Product, Set Category & Cost Prices")
            print("3️⃣ Update Sale Price for a Product")
            print("4️⃣ Update Sale Prices for a Shelf")
            print("5️⃣ Set/Update Product Category")
            print("6️⃣ Reset Cost Price for a Product")
            print("7️⃣ Get Min/Max Cost Price")
            print("8️⃣ Get Average Cost, Sale, and Profit for Shelf")
            print("9️⃣ Get Average Cost, Sale, and Profit for Product")
            print("0️⃣ Exit")
            ...
            if choice == "1":
                ...
                self.get_average_cost_sale_profit()
            elif choice == "9":
