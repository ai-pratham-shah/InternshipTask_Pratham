...
# Modify add_data function to include saving to Excel
def add_data():
    try:
...
# Update data in records and Excel
def update_data():
    try:
...
# Delete data from records and Excel
def delete_data():
    try:
...
def save_to_excel(records):
    try:
        ...
        for country, states in records.items():
            for state, cities in states.items():
                city_str = ", ".join(cities)  # Combine cities into a single string separated by commas
                ws.append([country, state, city_str])
